...
# Image dimension
image_size = 128
image_directory = "./Data/datasets/Faces/face_images/" # Directory of the images
# Load images
...
if not os.path.exists("./Data/datasets/Faces/augmented"):
    os.mkdir("./Data/datasets/Faces/augmented")
if not os.path.exists("./Data/datasets/Faces/test"):
    os.mkdir("./Data/datasets/Faces/test")
if not os.path.exists("./Data/datasets/Faces/L"):
    os.mkdir("./Data/datasets/Faces/L")
if not os.path.exists("./Data/datasets/Faces/a"):
    os.mkdir("./Data/datasets/Faces/a")
if not os.path.exists("./Data/datasets/Faces/b"):
    os.mkdir("./Data/datasets/Faces/b")

for i in range(augimages.shape[0]):
  cv2.imwrite("./Data/datasets/Faces/augmented/img_" + str(i) + ".jpg", cv2.cvtColor(augimages[i].astype('float32')*255, cv2.COLOR_RGB2BGR))
for i in range(npimages_test.shape[0]):
  cv2.imwrite("./Data/datasets/Faces/test/img_" + str(i) + ".jpg", cv2.cvtColor(npimages_test[i].astype('float32')*255, cv2.COLOR_RGB2BGR))
...
i = 0
for file in os.listdir("./Data/datasets/Faces/augmented/"):
  temp_img = cv2.imread("./Data/datasets/Faces/augmented/" + file)
...
i = 0
for file in os.listdir("./Data/datasets/Faces/test/"):
  temp_img = cv2.imread("./Data/datasets/Faces/test/" + file)
  lab_image = cv2.cvtColor(temp_img, cv2.COLOR_BGR2LAB)
  l,a,b = cv2.split(lab_image)
  l_images_ts[i] = l
  a_images_ts[i] = a
...
#Save Augmented Training Datasets for Submission
if not os.path.exists('./Data/arrays/Faces'):
    os.makedirs('./Data/arrays/Faces')

# Save numpy arrays for training
np.save('./Data/arrays/Faces/L_train.npy', l_images)
np.save('./Data/arrays/Faces/a_train.npy', a_images)
np.save('./Data/arrays/Faces/b_train.npy', b_images)
np.save('./Data/arrays/Faces/L_test.npy', l_images_ts)
np.save('./Data/arrays/Faces/a_test.npy', a_images_ts)
np.save('./Data/arrays/Faces/b_test.npy', b_images_ts)

# Save each channel to its own folder and zip them for submission
for i in range(augimages.shape[0]):
  cv2.imwrite("./Data/datasets/Faces/L/img_" + str(i) + ".jpg", l_images[i])
  cv2.imwrite("./Data/datasets/Faces/a/img_" + str(i) + ".jpg", a_images[i])
  cv2.imwrite("./Data/datasets/Faces/b/img_" + str(i) + ".jpg", b_images[i])
print('Done!')
